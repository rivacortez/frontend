...
export function useTables() {
  return useQuery({
    key: ["tables"],
    query: () =>
      $fetch<ApiResponse<DiningTable[]>>("/api/tables").then((r) => r.data),
  });
}
...
export function useTable(id: MaybeRefOrGetter<string>) {
  return useQuery({
    ...
    query: () =>
      $fetch<ApiResponse<TableWithOrder>>(`/api/tables/${toValue(id)}`).then(
        (r) => r.data,
      ),
  });
}
...
export function usePatchTable() {
  ...
  return useMutation({
    mutation: ({
      ...
      $fetch<ApiResponse<DiningTable>>(`/api/tables/${id}`, {
        method: "PATCH",
        body: payload,
      }).then((r) => r.data),
    onSettled: invalidate,
...
export function useAddOrderItems() {
  ...
  return useMutation({
    mutation: ({
      ...
      orderId: string;
      items: Array<{ recipeId: string; qty: number; notes?: string }>;
    }) =>
...
export function useSendToKitchen() {
  ...
  return useMutation({
    mutation: ({ orderId }: { orderId: string }) =>
      $fetch<ApiResponse<Order>>(`/api/orders/${orderId}/send-to-kitchen`, {
        method: "POST",
      }).then((r) => r.data),
    onSettled: invalidate,
  });
...
export function usePayOrder() {
  ...
  return useMutation({
    mutation: ({
      ...
      $fetch<ApiResponse<{ order: Order; sale: Sale }>>(
...
export function useTablesConfig() {
  return useQuery({
    ...
    query: () =>
      $fetch<ApiResponse<TableConfig[]>>("/api/tables/config").then(
        (r) => r.data,
      ),
  });
}
...
export function useCreateTable() {
  ...
  return useMutation({
    ...
    mutation: (payload: TableConfigPayload) =>
      $fetch<ApiResponse<TableConfig>>("/api/tables", {
        method: "POST",
        body: payload,
      }).then((r) => r.data),
    onSettled: invalidate,
...
export function useUpdateTableConfig() {
  ...
  return useMutation({
    ...
    mutation: ({
      id,
      ...payload
    }: Partial<TableConfigPayload> & { id: string }) =>
      $fetch<ApiResponse<TableConfig>>(`/api/tables/${id}/config`, {
        method: "PATCH",
        body: payload,
      }).then((r) => r.data),
    onSettled: invalidate,
...
export const TABLE_STATUS_VALUES: TableStatus[] = [
  "free",
  "occupied",
  "bill",
  "reserved",
...
// ===== Order upsell suggestions (Widget D) =====

/**
 * Sales-based upsell suggestions for an open order: top-sellers that are not
 * already in the order. NOT machine-learning — honest commercial analytics.
 *
 * Returns an empty array when `orderId` is null/undefined (safe to call before
 * the order is loaded). Also returns empty when the backend finds no candidates
 * (e.g. the order already contains all top-sellers, or no sales history exists).
 *
 * `recipeId` in each suggestion is resolved by the BFF from `menuItemId` via
 * the menu maps, so the POS cart can add items with no extra lookup.
 */
export function useOrderSuggestions(
  orderId: MaybeRefOrGetter<string | null | undefined>,
) {
  return useQuery({
    key: () => ["order-suggestions", toValue(orderId) ?? null] as const,
    query: () => {
      const id = toValue(orderId);
      if (!id) return Promise.resolve<OrderSuggestionView[]>([]);
      return $fetch<ApiResponse<OrderSuggestionView[]>>(
        `/api/orders/${id}/suggestions`,
      ).then((r) => r.data);
    },
  });
}
...
export function orderTotals(order: Order | null | undefined): {
  gross: number;
  discount: number;
  total: number;
} {
  ...
  const discount = order.discount
    ? order.discount.type === "pct"
      ? gross * (order.discount.value / 100)
