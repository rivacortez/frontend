...
function summaryToRecipe(
  ...
  const category = item
    ? item.menuCategoryId
      ? (catName.get(item.menuCategoryId) ?? "Sin categoría")
      : "Sin categoría"
    : SUB_RECIPE_CATEGORY;
  return {
    id: r.id,
    ...
    // HU-02-09: versionado de BOM. El backend lo incrementa en cada PATCH de items.
    version: r.version,
  };
}

export async function listRecipes(event: H3Event): Promise<Recipe[]> {
  const recipes = await backendFetch<Envelope<BeRecipeSummary[]>>(
    event,
    "/api/recipes",
  );
  const { itemByRecipe, catName } = await fetchMenuMaps(event);
  return recipes.data.map((r) =>
    summaryToRecipe(r, itemByRecipe.get(r.id), catName),
  );
}

// ---- Receta (detalle, con BOM completo) ----
export async function buildRecipeDetail(
  event: H3Event,
  id: string,
): Promise<Recipe> {
 ...
 */
function convertQtyToIngredientUnit(
  qty: number,
  from: string,
  to: string,
): number {
...
/** Shape que acepta el backend para cada línea del BOM (insumo O sub-receta). */
type ResolvedBomItem =
  | { ingredientId: string; qty: number; wasteFactor: number }
  | { subRecipeId: string; qty: number; wasteFactor: number };
 ...
 *
 * Soporta sub-recetas anidadas (HU-02-08): cuando `it.subRecipeId` está presente
 * se envía `{ subRecipeId, qty, wasteFactor }` en lugar de `{ ingredientId }`.
 */
async function resolveBackendItems(
  event: H3Event,
  items: RecipeItem[],
): Promise<ResolvedBomItem[]> {
  // Unidades de los insumos reales (UUID), para convertir qty a la unidad del insumo.
  // Excluir líneas que son sub-recetas (no tienen ingredientId real).
  const ingredientItems = items.filter(
    (it) => !it.subRecipeId && UUID_RE.test(it.ingredientId),
  );
  const ingUnitById = new Map<string, string>();
  if (ingredientItems.length > 0) {
    const ings = await backendFetch<Envelope<BeIngredient[]>>(
      event,
      "/api/ingredients",
    );
    for (const i of ings.data) ingUnitById.set(i.id, i.unit);
  }
  const out: ResolvedBomItem[] = [];
  for (const it of items) {
    const wasteFactor = (it.wastePct ?? 0) / 100;

    // HU-02-08: línea de sub-receta anidada — el backend calcula su costo recursivamente.
    if (it.subRecipeId) {
      out.push({ subRecipeId: it.subRecipeId, qty: it.qty, wasteFactor });
      continue;
    }
    ...
    if (!UUID_RE.test(ingredientId)) {
      ...
      const slug =
        it.name
          ...
          .replace(/[^A-Z0-9]+/g, "-")
          .slice(0, 32) || "INSUMO";
      const created = await backendFetch<Envelope<{ id: string }>>(
        event,
        "/api/ingredients",
        {
          method: "POST",
    ...
    out.push({ ingredientId, qty, wasteFactor });
  }
  return out;
}
...
async function resolveMenuCategoryId(
  ...
  const cats = await backendFetch<Envelope<BeMenuCategory[]>>(
    event,
    "/api/menu/categories",
  );
  const found = cats.data.find(
    (c) => c.name.toLowerCase() === name.toLowerCase(),
  );
  if (found) return found.id;
  const created = await backendFetch<Envelope<BeMenuCategory>>(
    event,
    "/api/menu/categories",
    {
      method: "POST",
...
export async function createRecipe(
  ...
  const recipe = await backendFetch<Envelope<BeRecipeView>>(
    event,
    "/api/recipes",
    {
      method: "POST",
...
export async function updateRecipe(
  ...
  if (payload.description !== undefined)
    recipeBody.description = payload.description;
  if (payload.prepMinutes !== undefined)
    recipeBody.prepMinutes = payload.prepMinutes;
  if (payload.items !== undefined)
    recipeBody.items = await resolveBackendItems(event, payload.items);
  ...
  if (touchesMenu) {
    ...
    if (item) {
      ...
      if (payload.category !== undefined) {
        menuBody.menuCategoryId = await resolveMenuCategoryId(
          event,
          payload.category,
        );
      }
      await backendFetch<Envelope<BeMenuItem>>(
        event,
...
export async function deleteRecipe(event: H3Event, id: string): Promise<void> {
  ...
  const item = items.data.find((it) => it.recipeId === id);
  if (item) {
    await backendFetch<Envelope<unknown>>(event, `/api/menu/items/${item.id}`, {
      method: "DELETE",
    });
  }
  await backendFetch<Envelope<unknown>>(event, `/api/recipes/${id}`, {
    method: "DELETE",
  });
}
...
export async function requireMenuItemId(
  event: H3Event,
  recipeId: string,
): Promise<string> {
  const items = await backendFetch<Envelope<BeMenuItem[]>>(
...
export async function listModifiers(
  ...
  const res = await backendFetch<Envelope<BeModifier[]>>(
    event,
    `/api/menu/items/${item.id}/modifiers`,
  );
...
export async function removeModifier(
  ...
  await backendFetch<Envelope<unknown>>(
    event,
    `/api/menu/modifiers/${modifierId}`,
    { method: "DELETE" },
  );
}
...
export async function listAvailability(
  ...
  const item = items.data.find((it) => it.recipeId === recipeId);
  if (!item) return [];
  const res = await backendFetch<Envelope<BeAvailability[]>>(
    event,
...
export async function removeAvailability(
  ...
  await backendFetch<Envelope<unknown>>(
    event,
    `/api/menu/availability/${availabilityId}`,
    { method: "DELETE" },
  );
}
